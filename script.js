...
/* ---- 3b. Ledger large preview: benchmark vs. actual savings line --
   Mirrors the real app's core idea: a straight-line benchmark from
   today to the deadline, plotted against an actual savings path that
   drifts above or below it. Ruled horizontal lines nod to the
   ledger/passbook visual style of the real site.                    */
(function ledgerPreview() {
  ...
  const POINTS = 12;
  const benchmark = Array.from({ length: POINTS }, (_, i) => i / (POINTS - 1));
  // an actual path that dips behind then pulls ahead of pace
  const actual = [0, 0.05, 0.11, 0.14, 0.19, 0.29, 0.34, 0.44, 0.58, 0.7, 0.83, 0.97];
  ...
  function draw() {
    ...
    // ledger ruled lines
    ctx.strokeStyle = 'rgba(98,195,214,0.06)';
    ctx.lineWidth = 1;
    const rows = 6;
    for (let r = 0; r <= rows; r++) {
      const y = padY + ((h - padY * 2) / rows) * r;
      ctx.beginPath();
      ctx.moveTo(padX, y);
      ctx.lineTo(w - padX, y);
      ctx.stroke();
    ...
    if (!prefersReducedMotion) progress += 0.006;
    if (progress > 1.3) progress = 0;
    const shown = Math.min(1, Math.max(0, progress));

    const xAt = (i) => padX + (i / (POINTS - 1)) * (w - padX * 2);
    ...
    function drawLine(series, color, dashed) {
      ...
      const count = Math.max(1, Math.floor(shown * (POINTS - 1)) + 1);
      ...
      if (last >= 0) {
        ctx.beginPath();
        ctx.fillStyle = color;
        ctx.arc(xAt(last), yAt(series[last]), 3.5, 0, Math.PI * 2);
        ctx.fill();
      }
    }

    drawLine(benchmark, '#3c7f8d', true);
    drawLine(actual, '#e5a445', false);

    // legend
    ctx.font = '500 11px "JetBrains Mono", monospace';
    ctx.textBaseline = 'middle';
    ctx.fillStyle = '#3c7f8d';
    ctx.fillText('- - benchmark', padX, padY - 14);
    ctx.fillStyle = '#e5a445';
    ctx.fillText('\u2014 actual', padX + 108, padY - 14);
...
/* ---- 3c. Emberdeep small preview: ambient torch-lit tiles ---- */
...
/* ---- 3d. Cube timer preview: ticking millisecond readout ---- */
...
/* ---- 3e. Reaction game preview: pulsing target + go/wait state --- */
